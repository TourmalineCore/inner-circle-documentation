...
- Minimize overlap of tests to reduce the efforts that we put into their writing and maintenance while keeping their high value. Tests are code, it has its price, we need to be maximally efficient yet reliable and automatically testable.
...
All tests must support concurrent parallel invocation. Thus, tests don't rely on each other's resulsts, they don't wait each other as well and their execution complete faster.
...
#### Why we write these tests using Karate?

We want them to be stack independent. Thus, you can write a backend servie using any framework and language like .NET(C#), NestJS(JS), FastAPI(Python) or even C++ but the E2E tests language is the same.
...
Karate scenario is a unit of parallelism of this type of tests. Scenarios are being executed in parallel. Thus, scenarios should be isolated from each other using different Tenants, Accounts, and more custom approaches. For instance, in time-api we can isonalate different scenarios using different week days, Monday for one scenario and Tuesday for another one.  

#### Production Execution

All E2E tests must support execution against Prod and Local. 

However, there might be cases when some E2E tests cannot be executed against Local Env. For instance, that was the case at testing books-ui URL redirection logic that couldn't be tested locally because it was a DNS feature.

#### What we test

We test only happy path - the core business flow. We limit the number of E2E tests to only the core business flow to avoid falling back into the situation when we test every change with a coarse-grained tests when it is an overkill and a lighter more fine-grained test is enough. Time to run the tests is also a limitation factor yet minor at this phase. Tests against Prod are being executed fater than 5 mins, it seems ok.
