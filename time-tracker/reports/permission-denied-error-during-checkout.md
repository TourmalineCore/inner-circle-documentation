...
## Cause of error
The problem occurs because, when using a DooD self-hosted runner together with the GitHub Actions container feature, the job container is created by the host Docker daemon, while the runner itself runs inside a container as the `runner` user. The working directory therefore has to be mounted from the host into the job container.
