...
## Context
We need to set up automatic email notifications to corporate mail for employees. These notifications should be sent when someone creates, updates, or deletes any of these events: away with make-up time, sick leave, vacation, or other events.
...
## Decision 
After conducting some research, we concluded that the solution involving email notifications—which had seemed the simplest at the time—no longer appears to be faster or easier than implementing this functionality directly in IC.

The email solution was not intended to be a permanent one; it was meant to be a temporary measure that would eventually need to be migrated to IC.

With email, it’s not possible (using simple means) to tell which employees have read a notification, unlike with our own service, where we can implement whatever we want.

Grouping messages based on the same action (creation, editing, deletion) is easier to implement in our own solution than trying to use email mechanisms for this purpose.

For now, the simplest solution is to display all notifications in a table using server-side pagination.
...
We create a dedicated mailbox (e.g., notifications@corporatemail.com). The time-tracker sends all notifications to this single mailbox. Employees are then given read access to it.
...
## Use the "Mailing" tool
Mailing are an additional feature of the mailbox for mass corporate email campaigns. This tool is well-suited for marketing purposes (e.g., mass mailing of news to clients, promotions, etc.), but not for automatic internal company notifications.
