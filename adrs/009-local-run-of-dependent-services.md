...
3. `npm run local-services:up` runs `prepare-local-run` to download the compose files (and, for books-api, the mock server config) from GitHub as described in [008](./008-downloading-files-for-local-run.md), then starts books-api and layout-ui as containers
